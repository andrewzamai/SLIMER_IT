"""
Evaluate SLIMER-PARALLEL model for zero-shot NER on italian datasets
...
def load_or_build_dataset_SLIMER_format(
    datasets_cluster_name, subdataset_name, data_handler, with_definition, max_tagNames_per_prompt, mask_labels
):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='''Evaluate SLIMER-IT-PARALLEL (w/ GRPO reasoning) Zero-Shot NER performance''')
    ...
    parser.add_argument('--mask_labels', action='store_true', help='Whether to use Def & Guidelines')
    ...
    for data in to_eval_on:
        ...
        for subdataset_name in data['subdataset_names']:
            ...
            def format_chat_template(row):
                ...
                row["prompt"] = tokenizer.apply_chat_template(conversation, tokenize=False, truncation=True, max_length=cutoff_len, add_generation_prompt=True) #, enable_thinking=True) #+ "\n\n<think>\n"
                return row
