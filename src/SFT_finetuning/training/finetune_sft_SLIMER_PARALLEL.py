...
if __name__ == "__main__":
    ...
    dataset_name = f"KIND_WN_only_PARALLEL_ALL_wDeG"
    ...
    parallel_datasetDict = dataset_KIND_manager.convert_dataset_for_SLIMER_PARALLEL(
        exclude_misc=True,
        mask_labels=True, # train masking labels
    ...
    if not os.path.exists(f"./datasets/KIND/SLIMER/{dataset_name}"):
        for split_name, dataset in parallel_datasetDict.items():
    ...
    configs['output_dir'] = f"./trained_models/SLIMER-IT-PARALLEL-LLaMAntino3"
